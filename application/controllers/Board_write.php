...
class Board_write extends CB_Controller
{
	 ...
	 * 모델을 로딩합니다
	 */
	protected $models = array('Post', 'Post_link', 'Post_file', 'Post_extra_vars', 'Post_meta', 'CIC_forum', 'CIC_forum_config');
	...
	public function _write_common($board, $origin = '', $reply = '')
	{
		...
		if ($this->session->userdata('lastest_post_time') && $this->cbconfig->item('new_post_second')) {
			if ($this->session->userdata('lastest_post_time') >= ( ctimestamp() - $this->cbconfig->item('new_post_second')) && $is_admin === false) {
				alert('너무 빠른 시간내에 게시물을 연속해서 올릴 수 없습니다.' . ($this->cbconfig->item('new_post_second') - (ctimestamp() - $this->session->userdata('lastest_post_time'))) . '초 후 글쓰기가 가능합니다');
			}
		}
		...
		if ($form_validation === false OR $file_error) {
			...
			if ($form && is_array($form)) {
				foreach ($form as $key => $value) {
					...
					if(element('field_name', $value) === 'A_opinion' || element('field_name', $value) === 'B_opinion'){
	...
	public function modify($post_id = 0)
	{
		...
		if($board_id == 3){
			...
			if($this->input->get('type') == 1 && $this->member->is_admin() != 'super'){
				alert('권한이 없습니다');
				return false;
