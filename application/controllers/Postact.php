			...
			switch(element('brd_id', $post)){
				case 1:{
					...
					if(element('lik_type', $exist) == 1){
						//추천의 경우
						$vp_free_recommended_point = element('vpc_value' ,$this->CIC_vp_config_model->get_one('','',"vpc_id = 6 AND vpc_enable = 1"));
						$vp_free_recommender_point = element('vpc_value' ,$this->CIC_vp_config_model->get_one('','',"vpc_id = 8 AND vpc_enable = 1"));
						$cp_free_recommended_point = element('cpc_value' ,$this->CIC_cp_config_model->get_one('','',"cpc_id = 6 AND cpc_enable = 1"));
						$cp_free_recommender_point = element('cpc_value' ,$this->CIC_cp_config_model->get_one('','',"cpc_id = 8 AND cpc_enable = 1"));
						if($vp_free_recommended_point){
							$give_point = floor( $usedPoint * $vp_free_recommended_point );
						...
						if($vp_free_recommender_point){
							$give_point = floor( $usedPoint * $vp_free_recommender_point );
							$this->point->insert_vp(
								$mem_id,
								$give_point,
						...
						if($cp_free_recommended_point){
							$give_point = floor( $usedPoint * $cp_free_recommended_point * 100 ) / 100;
						...
						if($cp_free_recommender_point){
							$give_point = floor( $usedPoint * $cp_free_recommender_point * 100 ) / 100;
							$this->point->insert_cp(
								$mem_id,
								$give_point,
								element('board_name', $board) . ' ' . $give_point . ' 추천함',
						...
						//비추천의 경우
						$vp_free_not_recommended_point = element('vpc_value' ,$this->CIC_vp_config_model->get_one('','',"vpc_id = 7 AND vpc_enable = 1"));
						$vp_free_not_recommender_point = element('vpc_value' ,$this->CIC_vp_config_model->get_one('','',"vpc_id = 9 AND vpc_enable = 1"));
						$cp_free_not_recommended_point = element('cpc_value' ,$this->CIC_cp_config_model->get_one('','',"cpc_id = 7 AND cpc_enable = 1"));
						$cp_free_not_recommender_point = element('cpc_value' ,$this->CIC_cp_config_model->get_one('','',"cpc_id = 9 AND cpc_enable = 1"));
						if($vp_free_not_recommended_point){
							$give_point = floor( $usedPoint * $vp_free_not_recommended_point );
							$this->point->insert_vp(
								$mem_id,
								$give_point,
								element('board_name', $board) . ' ' . $give_point . ' 비추천받음',
								'post_recommended',
								$post_id,
								'게시글 비추천받음'
							);
						}
						if($vp_free_not_recommender_point){
							$give_point = floor( $usedPoint * $vp_free_not_recommender_point );
							$this->point->insert_vp(
								$mem_id,
								$give_point,
								element('board_name', $board) . ' ' . $give_point . ' 비추천함',
								'post_recommender',
								$post_id,
								'게시글 비추천함'
							);
						}
						if($cp_free_not_recommended_point){
							$give_point = floor( $usedPoint * $cp_free_not_recommended_point * 100 ) / 100;
							$this->point->insert_cp(
								$mem_id,
								$give_point,
								element('board_name', $board) . ' ' . $give_point . ' 비추천받음',
						...
						if($cp_free_not_recommender_point){
							$give_point = floor( $usedPoint * $cp_free_not_recommender_point * 100 ) / 100;
							$this->point->insert_cp(
								$mem_id,
								$give_point,
								element('board_name', $board) . ' ' . $give_point . ' 비추천함',
				...
				case 2:{
					...
					if(element('lik_type', $exist) == 1){
						//추천의 경우
						$vp_writer_recommended_point = element('vpc_value' ,$this->CIC_vp_config_model->get_one('','',"vpc_id = 14 AND vpc_enable = 1"));
						$vp_writer_recommender_point = element('vpc_value' ,$this->CIC_vp_config_model->get_one('','',"vpc_id = 16 AND vpc_enable = 1"));
						$cp_writer_recommended_point = element('cpc_value' ,$this->CIC_cp_config_model->get_one('','',"cpc_id = 14 AND cpc_enable = 1"));
						$cp_writer_recommender_point = element('cpc_value' ,$this->CIC_cp_config_model->get_one('','',"cpc_id = 16 AND cpc_enable = 1"));
						if($vp_writer_recommended_point){
							$give_point = floor( $usedPoint * $vp_writer_recommended_point );
							$this->point->insert_vp(
								$mem_id,
								$give_point,
								element('board_name', $board) . ' ' . $give_point . ' 추천받음',
						...
						if($vp_writer_recommender_point){
							$give_point = floor( $usedPoint * $vp_writer_recommender_point );
							$this->point->insert_vp(
								$mem_id,
								$give_point,
								element('board_name', $board) . ' ' . $give_point . ' 추천함',
						...
						if($cp_writer_recommended_point){
							$give_point = floor( $usedPoint * $cp_writer_recommended_point * 100 ) / 100;
							$this->point->insert_cp(
								$mem_id,
								$give_point,
								element('board_name', $board) . ' ' . $give_point . ' 추천받음',
						...
						if($cp_writer_recommender_point){
							$give_point = floor( $usedPoint * $cp_writer_recommender_point * 100 ) / 100;
							$this->point->insert_cp(
								$mem_id,
								$give_point,
								element('board_name', $board) . ' ' . $give_point . ' 추천함',
						...
						//비추천의 경우
						$vp_writer_not_recommended_point = element('vpc_value' ,$this->CIC_vp_config_model->get_one('','',"vpc_id = 15 AND vpc_enable = 1"));
						$vp_writer_not_recommender_point = element('vpc_value' ,$this->CIC_vp_config_model->get_one('','',"vpc_id = 17 AND vpc_enable = 1"));
						$cp_writer_not_recommended_point = element('cpc_value' ,$this->CIC_cp_config_model->get_one('','',"cpc_id = 15 AND cpc_enable = 1"));
						$cp_writer_not_recommender_point = element('cpc_value' ,$this->CIC_cp_config_model->get_one('','',"cpc_id = 17 AND cpc_enable = 1"));
						if($vp_writer_not_recommended_point){
							$give_point = floor( $usedPoint * $vp_writer_not_recommended_point );
							$this->point->insert_vp(
								$mem_id,
								$give_point,
								element('board_name', $board) . ' ' . $give_point . ' 비추천받음',
						...
						if($vp_writer_not_recommender_point){
							$give_point = floor( $usedPoint * $vp_writer_not_recommender_point );
							$this->point->insert_vp(
								$mem_id,
								$give_point,
								element('board_name', $board) . ' ' . $give_point . ' 비추천함',
						...
						if($cp_writer_not_recommended_point){
							$give_point = floor( $usedPoint * $cp_writer_not_recommended_point * 100 ) / 100;
							$this->point->insert_cp(
								$mem_id,
								$give_point,
								element('board_name', $board) . ' ' . $give_point . ' 비추천받음',
						...
						if($cp_writer_not_recommender_point){
							$give_point = floor( $usedPoint * $cp_writer_not_recommender_point * 100 ) / 100;
							$this->point->insert_cp(
								$mem_id,
								$give_point,
								element('board_name', $board) . ' ' . $give_point . ' 비추천함',
		...
		if(in_array(element('brd_id', $post), array(1, 2))){
			switch(element('brd_id', $post)){
				case 1:{
					...
					if(element('lik_type', $exist) == 1){
						//추천의 경우
						$vp_free_recommended_point = element('vpc_value' ,$this->CIC_vp_config_model->get_one('','',"vpc_id = 10 AND vpc_enable = 1"));
						$vp_free_recommender_point = element('vpc_value' ,$this->CIC_vp_config_model->get_one('','',"vpc_id = 12 AND vpc_enable = 1"));
						$cp_free_recommended_point = element('cpc_value' ,$this->CIC_cp_config_model->get_one('','',"cpc_id = 10 AND cpc_enable = 1"));
						$cp_free_recommender_point = element('cpc_value' ,$this->CIC_cp_config_model->get_one('','',"cpc_id = 12 AND cpc_enable = 1"));
						if($vp_free_recommended_point){
							$give_point = floor( $usedPoint * $vp_free_recommended_point );
							$this->point->insert_vp(
								$mem_id,
						...
						//비추천의 경우
						$vp_free_not_recommended_point = element('vpc_value' ,$this->CIC_vp_config_model->get_one('','',"vpc_id = 11 AND vpc_enable = 1"));
						$vp_free_not_recommender_point = element('vpc_value' ,$this->CIC_vp_config_model->get_one('','',"vpc_id = 13 AND vpc_enable = 1"));
						$cp_free_not_recommended_point = element('cpc_value' ,$this->CIC_cp_config_model->get_one('','',"cpc_id = 11 AND cpc_enable = 1"));
						$cp_free_not_recommender_point = element('cpc_value' ,$this->CIC_cp_config_model->get_one('','',"cpc_id = 13 AND cpc_enable = 1"));
						if($vp_free_not_recommended_point){
							$give_point = floor( $usedPoint * $vp_free_not_recommended_point );
							$this->point->insert_vp(
								$mem_id,
				...
				case 2:{
					...
					if(element('lik_type', $exist) == 1){
						//추천의 경우
						$vp_writer_recommended_point = element('vpc_value' ,$this->CIC_vp_config_model->get_one('','',"vpc_id = 18 AND vpc_enable = 1"));
						$vp_writer_recommender_point = element('vpc_value' ,$this->CIC_vp_config_model->get_one('','',"vpc_id = 20 AND vpc_enable = 1"));
						$cp_writer_recommended_point = element('cpc_value' ,$this->CIC_cp_config_model->get_one('','',"cpc_id = 18 AND cpc_enable = 1"));
						$cp_writer_recommender_point = element('cpc_value' ,$this->CIC_cp_config_model->get_one('','',"cpc_id = 20 AND cpc_enable = 1"));
						if($vp_writer_recommended_point){
							$give_point = floor( $usedPoint * $vp_writer_recommended_point );
							$this->point->insert_vp(
								$mem_id,
						...
						//비추천의 경우
						$vp_writer_not_recommended_point = element('vpc_value' ,$this->CIC_vp_config_model->get_one('','',"vpc_id = 19 AND vpc_enable = 1"));
						$vp_writer_not_recommender_point = element('vpc_value' ,$this->CIC_vp_config_model->get_one('','',"vpc_id = 21 AND vpc_enable = 1"));
						$cp_writer_not_recommended_point = element('cpc_value' ,$this->CIC_cp_config_model->get_one('','',"cpc_id = 19 AND cpc_enable = 1"));
						$cp_writer_not_recommender_point = element('cpc_value' ,$this->CIC_cp_config_model->get_one('','',"cpc_id = 21 AND cpc_enable = 1"));
						if($vp_writer_not_recommended_point){
							$give_point = floor( $usedPoint * $vp_writer_not_recommended_point );
							$this->point->insert_vp(
								$mem_id,
