 ...
 */
class CICPoints extends CB_Controller
{
	 ...
	 * 페이지 이동시 필요한 정보입니다
	 */
	public $pagedir = 'member/CICPoints';
	...
	public function index()
	{
		 ...
		 */
		$view['view']['primary_key'] = $this->CIC_vp_model->primary_key;
		 ...
		 */
		$search_option = array('vp_datetime' => '날짜', 'vp_content' => '내용', 'mem_nickname' => '닉네임');
	...
	public function cp()
	{
		...
		$_modelname = 'CIC_cp_model';
		$_table = 'cic_cp';

		$param =& $this->querystring;
		$page = (((int) $this->input->get('page')) > 0) ? ((int) $this->input->get('page')) : 1;
	
		$findex = $this->input->get('findex') ? $this->input->get('findex') : $this->{$_modelname}->primary_key;
		$forder = $this->input->get('forder', null, 'desc');
		$sfield = $this->input->get('sfield', null, '');
		$skeyword = $this->input->get('skeyword', null, '');
		 ...
		 * primary key 정보를 저장합니다
		 */
		$view['view']['primary_key'] = $this->CIC_cp_model->primary_key;
		 ...
		 * 쓰기 주소, 삭제 주소등 필요한 주소를 구합니다
		 */
		$search_option = array('cp_datetime' => '날짜', 'cp_content' => '내용', 'mem_nickname' => '닉네임');
		$view['view']['skeyword'] = ($sfield && array_key_exists($sfield, $search_option)) ? $skeyword : '';
		$view['view']['search_option'] = search_option($search_option, $sfield);
		$view['view']['listall_url'] = admin_url($this->pagedir);
	...
	public function write()
	{
		...
		if ($this->form_validation->run() === false) {
			...
			$_modelname = $pointType === 'vp' ? 'CIC_vp_model' : 'CIC_cp_model';
