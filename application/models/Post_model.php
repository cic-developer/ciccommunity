...
class Post_model extends CB_Model
{
	...
	public function get_post_list($limit = '', $offset = '', $where = '', $category_id = '', $orderby = '', $forder = 'asc', $sfield = '', $skeyword = '', $sop = 'OR')
	{
		...
		$this->db->select('post.*, post.mem_id, member.mem_userid, member.mem_nickname, member.mem_icon, member.mem_photo, member.mem_point, cic_member_level_config.*');
	...
	public function upadte_post_best_state($post_id)
	{
		...
		public function upadte_post_best_state_exept($post_id)
	{
		$where = array(
			'post_id' => $post_id,
		);
	...
	// 포럼 관련 데이터를 join  하기 위하여 만들어진 함수입니다.
