...
var mainNavi_W = {
	...
	subMenuBarClose:function(){
		...
		$("#mainNavi-wrap").stop().animate({height:66},300,function(){});
		$(".mn-bar").stop().animate({"height":0},300,function(){$(this).hide();});
		$('#header-wrap').removeClass('isOver');
		$(".topmenu").find(".mn_l1").removeClass('is-over is-open');
	},
...
var mainNavi_H = {
	...
	_init:function(){
		...
		$("li",$(this.mnObj)).each(function(){
			var sLI = $("ul",$(this));
			if(sLI.length>0){
				if($(this).find(" > .tgl-btn").length<1){ 		$(this).prepend("<button class='tgl-btn' type='button'>ToggleMenu<span></span></button>");		}
...
var mainNavi = {
	...
	_init:function(mn,initNum){
		...
		$(".bt-mnall").off("click").click(function(){
